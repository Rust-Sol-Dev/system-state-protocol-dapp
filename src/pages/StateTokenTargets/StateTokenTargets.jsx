...
import './StateTokenTargets.css'

export default function StateTokenTarget() {
    ...
    const { socket, getStateTokenTargets, getStateTokenHolders } = useContext(functionsContext)
        ...
        if (accountAddress) {
            try {
                ...
                for (let index = 0; index < allStateTokensAddress.length; index++) {
                    ...
                    console.log('stateTargetValue', targets)
                }

                const sortedArray = [...All_USERS_TARGETS || []].sort((a, b) => {
                    const formattedRatioTargetA = ethers.utils.formatEther(a?.Time.toString());
                    const formattedRatioTargetB = ethers.utils.formatEther(b?.Time.toString());
                    console.log('numericValueB', a?.Time);
    ...
    console.log('ratioPriceTargets:',ratioPriceTargets);

    return (
        <>
            {/* {ratioPriceTargets} */}

            <div className="nextClassDiv">
                ...
                <div
                    ...
                    <div className={`box-title mx-3 ${theme === "darkTheme" && ""} `} id={``} >
                        <h1 className={`box-title mb-3 ${(theme === "darkTheme" && "bg-dark" && "text-white") || (theme === "dimTheme" && "title-color")}`}>
                            State Tokens Transaction History
                        </h1>
                    </div>
